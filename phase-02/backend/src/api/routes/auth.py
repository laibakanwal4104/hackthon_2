...
async def sign_up(
    ...
    user = User(email=request.email, hashed_password=password_hash)
    session.add(user)
    session.commit()
    ...
    print(f"DEBUG: UserResponse created: {user_response}")
    ...
    print(f"DEBUG: SignUpResponse created: {signup_response}")
